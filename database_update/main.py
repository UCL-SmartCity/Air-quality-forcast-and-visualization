...
def update_air(mo, mydb):
    print('Updating air quality data...')
    mycol = mydb['PM25Readings']
    mydoc = mycol.find({'sensorid': 'WMD-PM25'}
                       ).sort('recordtime', pymongo.DESCENDING).limit(1)
    r = list(mydoc)
    last_update = datetime.datetime.strptime(
        r[0]['recordtime'], "%Y-%m-%d %H:%M:%S")
    ...
    for i in range(106):
        if mo.loc[i]['NO2'] == 1:
            ...
            for item in li['RawAQData']['Data']:
                ...
                recordtime = datetime.datetime.strptime(
                    rt, "%Y-%m-%d %H:%M:%S")
...
def update_weather(mydb):
    ...
    last_update = datetime.datetime.strptime(
        latest_data[0]['recordtime'], "%Y-%m-%d %H:%M:%S")

    with open('database_update/weather/weather.json') as data:
...
if __name__ == '__main__':
    ...
    while (1):
        update_air(monitoring, mydb)
        update_weather(mydb)
        time.sleep(21600) # update every six hours
